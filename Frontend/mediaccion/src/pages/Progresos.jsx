...
const ProgressScreenContent = () => {
    ...
    const [vista, setVista] = useState('progresos'); // 'premios' | 'progresos'
    const [tipoVista, setTipoVista] = useState(true); // 'premios' | 'progresos'
    const [currentDate, setCurrentDate] = useState(new Date()); // month selector
    const [selectedMed, setSelectedMed] = useState(null); // nombre del medicamento seleccionado en el pie
    ...
    // Vista de estadísticas / progresos con medicamentos reales
    const ProgressView = () => {
        // Obtener todos los medicamentos únicos de todas las fechas
        const allMeds = [];
        Object.values(medicamentos).forEach(dayMeds => {
            dayMeds.forEach(med => {
                // Evitar duplicados por nombre
                if (!allMeds.find(m => m.nombre === med.nombre)) {
                    allMeds.push(med);
                }
            });
        });
        ...
        const totalTomas = allMeds.reduce((sum, m) => sum + (m.total_tomas || 1), 0);
        const tomasCumplidas = allMeds.reduce((sum, m) => sum + (m.tomadas || 0), 0);
        ...
        const medsThisMonth = [];
        Object.entries(medicamentos).forEach(([fecha, arr]) => {
            // fecha expected format YYYY-MM-DD
            const d = new Date(fecha);
            if (d.getFullYear() === year && d.getMonth() === month) {
                arr.forEach(m => medsThisMonth.push(m));
            }
        });
        ...
        const items = Object.keys(byName).map(name => ({
            nombre: name,
        ...
        medsThisMonth.forEach(m => {
            const missed = (m.total_tomas || 0) - (m.tomadas || 0);
            if (missed > 0) {
                forgottenDetails.push({ nombre: m.nombre, fecha: m.fecha, missed });
            }
        });
